...
@Component({
    ...
    template: `
      ...
      <a *ngIf="(item.routerLink && !item.items) && item.visible !== false" (click)="itemClick($event)" [ngClass]="item.class"
               ...
               [attr.target]="item.target" tabindex="0" pRipple class="flex-grow-1">
				<i [ngClass]="item.icon" class="layout-menuitem-icon"></i>
				<span class="layout-menuitem-text">{{item.label}}</span>
				<i class="pi pi-fw pi-angle-down layout-submenu-toggler" *ngIf="item.items"></i>
      ...
      <p-button  *ngIf="item.deletable" icon="pi pi-times" (click)="deleteMenu(item)" [rounded]="true" [text]="true" severity="danger " />
...
export class AppMenuitemComponent implements OnInit, OnDestroy {
    ...
    deleteMenu(item: MenuItemLocalStorage) {
     this.menuService.removeItem(item);
     this.menuService.onMenuDeleted(item.id);
    }
}
